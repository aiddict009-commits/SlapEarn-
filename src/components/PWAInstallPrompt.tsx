...
export const PWAInstallPrompt: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Also show banner for iOS users if not standalone
    if (isIosDevice) {
      setShowPrompt(true);
    }
  ...
  return (
    <>
      {/* Floating PWA Install Banner at Top / Bottom */}
      <div className="fixed bottom-20 left-3 right-3 sm:left-auto sm:right-6 sm:bottom-6 sm:max-w-sm z-50 bg-[#0F172A] border-3 border-[#00D09E] rounded-2xl p-3.5 shadow-[0_8px_25px_rgba(0,0,0,0.5)] animate-bounce-short">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-300 mt-0.5">
                Faster slaps, offline access & instant cashouts!
              </p>
            </div>
          </div>
      ...
      {showIOSGuide && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#101426] border-4 border-amber-400 rounded-2xl max-w-sm w-full p-5 text-white shadow-[6px_6px_0px_0px_#000]">
            ...
            <button
              onClick={() => setShowIOSGuide(false)}
              className="w-full bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs py-2.5 rounded-xl border-2 border-slate-950 shadow-[2px_2px_0px_0px_#000]"
            >
              Got It!
            </button>
          </div>
